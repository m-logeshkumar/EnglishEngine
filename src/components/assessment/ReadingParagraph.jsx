import { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAudioRecorder } from '../../hooks/useAudioRecorder';
import { useSpeechRecognition } from '../../hooks/useSpeechRecognition';
import { analyzeReading } from '../../services/aiService';
import { getParagraphs } from '../../services/dataService';
import { Mic, Square, Play, RotateCcw, Send, BookOpen, Loader2, ChevronRight, RefreshCw, AlertCircle } from 'lucide-react';

export default function ReadingParagraph({ onComplete }) {
  const [paragraph, setParagraph] = useState(null);
  const [paragraphs, setParagraphs] = useState([]);
  const [analyzing, setAnalyzing] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [result, setResult] = useState(null);
  const [submitted, setSubmitted] = useState(false);
  const [error, setError] = useState(null);
  const [isLoadingParagraphs, setIsLoadingParagraphs] = useState(true);
  const [retryCount, setRetryCount] = useState(0);
  const mountedRef = useRef(true);
  
  const { isRecording, audioBlob, audioUrl, duration, startRecording, stopRecording, clearRecording, formatDuration } = useAudioRecorder();
  const { transcript, isListening, startListening, stopListening, resetTranscript } = useSpeechRecognition();

  // Cleanup on unmount
  useEffect(() => {
    mountedRef.current = true;
    return () => {
      mountedRef.current = false;
      // Cleanup speech recognition
      stopListening();
      // Cleanup any speech synthesis
      if (window.speechSynthesis) {
        window.speechSynthesis.cancel();
      }
    };
  }, [stopListening]);

  // Load paragraphs
  useEffect(() => {
    let isMounted = true;
    
    async function loadParagraphs() {
      try {
        setIsLoadingParagraphs(true);
        setError(null);
        const loaded = await getParagraphs();
        if (!isMounted) return;
        
        setParagraphs(Array.isArray(loaded) ? loaded : []);
        if (Array.isArray(loaded) && loaded.length > 0) {
          const randomIndex = Math.floor(Math.random() * loaded.length);
          setParagraph(loaded[randomIndex]);
        } else {
          setError('No reading exercises available. Please refresh the page.');
        }
      } catch (err) {
        console.error('Failed to load paragraphs:', err);
        if (isMounted) {
          setError('Failed to load reading exercises. Please refresh the page.');
        }
      } finally {
        if (isMounted) {
          setIsLoadingParagraphs(false);
        }
      }
    }

    loadParagraphs();

    return () => {
      isMounted = false;
    };
  }, []);

  const handleStartRecording = useCallback(async () => {
    setError(null);
    resetTranscript();
    await startRecording();
    startListening();
  }, [startRecording, startListening, resetTranscript]);

  const handleStopRecording = useCallback(() => {
    stopRecording();
    stopListening();
  }, [stopRecording, stopListening]);

  const handleReRecord = useCallback(() => {
    clearRecording();
    resetTranscript();
    setResult(null);
    setSubmitted(false);
    setError(null);
    setRetryCount(0);
  }, [clearRecording, resetTranscript]);

  const handleNewParagraph = useCallback(() => {
    if (paragraphs.length === 0) {
      setError('No paragraphs available. Please refresh the page.');
      return;
    }
    const randomIndex = Math.floor(Math.random() * paragraphs.length);
    setParagraph(paragraphs[randomIndex]);
    handleReRecord();
  }, [paragraphs, handleReRecord]);

  const handleSubmit = useCallback(async () => {
    if (!audioBlob || !transcript || isSubmitting || submitted) return;
    
    setAnalyzing(true);
    setIsSubmitting(true);
    setError(null);
    setSubmitted(true);
    
    try {
      const res = await analyzeReading(transcript, paragraph?.text || '');
      if (!mountedRef.current) return;
      
      setResult(res);
      if (onComplete) onComplete(res);
    } catch (err) {
      console.error('Reading analysis failed:', err);
      if (mountedRef.current) {
        setError('Failed to analyze your reading. Please try again.');
        setSubmitted(false);
        setRetryCount(prev => prev + 1);
      }
    } finally {
      if (mountedRef.current) {
        setAnalyzing(false);
        setIsSubmitting(false);
      }
    }
  }, [audioBlob, transcript, isSubmitting, submitted, paragraph, onComplete]);

  const handleRetry = useCallback(() => {
    setError(null);
    setRetryCount(0);
    handleReRecord();
  }, [handleReRecord]);

  const renderWordHighlights = useCallback(() => {
    if (!result || !paragraph) return paragraph?.text || '';
    
    const words = paragraph.text.split(/(\s+)/);
    const missedWords = Array.isArray(result.missedWords) ? result.missedWords : [];
    const mispronounced = Array.isArray(result.mispronounced) ? result.mispronounced : [];
    
    return words.map((word, i) => {
      if (/^\s+$/.test(word)) return word;
      const clean = word.toLowerCase().replace(/[^\w]/g, '');
      const isMispronounced = mispronounced.some(m => m.expected === clean);
      const isMissed = missedWords.includes(clean);
      return (
        <span key={i} className={`word ${isMispronounced ? 'incorrect' : isMissed ? 'missed' : submitted ? 'correct' : ''}`}>
          {word}
        </span>
      );
    });
  }, [result, paragraph, submitted]);

  if (isLoadingParagraphs) {
    return (
      <div style={{ textAlign: 'center', padding: '40px' }}>
        <Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: 'var(--accent-blue)' }} />
        <p style={{ color: 'var(--text-secondary)', marginTop: 12 }}>Loading reading exercises...</p>
      </div>
    );
  }

  if (error && !paragraph && !isLoadingParagraphs) {
    return (
      <div className="glass-card" style={{ padding: 40, textAlign: 'center' }}>
        <AlertCircle size={48} style={{ color: 'var(--accent-rose)', marginBottom: 16 }} />
        <h3 style={{ marginBottom: 12 }}>Unable to Load Reading Exercises</h3>
        <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>{error}</p>
        <button className="btn btn-primary" onClick={handleNewParagraph}>
          <RefreshCw size={16} /> Try Again
        </button>
      </div>
    );
  }

  if (!paragraph) {
    return (
      <div className="glass-card" style={{ padding: 40, textAlign: 'center' }}>
        <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>No reading exercises available.</p>
        <button className="btn btn-primary" onClick={handleNewParagraph}>
          <RefreshCw size={16} /> Refresh
        </button>
      </div>
    );
  }

  return (
    <div style={{ animation: 'fadeInUp 0.5s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h2><BookOpen size={24} style={{ verticalAlign: 'middle', marginRight: 8 }} />Reading Paragraph</h2>
          <p style={{ color: 'var(--text-secondary)' }}>Read the paragraph below aloud clearly</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <span className={`badge badge-${paragraph.difficulty === 'Advanced' ? 'rose' : paragraph.difficulty === 'Intermediate' ? 'amber' : 'emerald'}`}>
            {paragraph.difficulty || 'Beginner'}
          </span>
          <button 
            className="btn btn-ghost btn-sm" 
            onClick={handleNewParagraph}
            disabled={isSubmitting || analyzing}
          >
            <RefreshCw size={14} /> New Paragraph
          </button>
        </div>
      </div>

      {/* Error Message */}
      {error && (
        <div className="glass-card" style={{ padding: 16, marginBottom: 16, borderColor: 'var(--accent-rose)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--accent-rose)' }}>
            <AlertCircle size={20} />
            <p style={{ margin: 0, fontSize: '0.9rem' }}>{error}</p>
            {retryCount < 3 && (
              <button className="btn btn-ghost btn-sm" onClick={handleRetry} style={{ marginLeft: 'auto' }}>
                Retry
              </button>
            )}
          </div>
        </div>
      )}

      {/* Paragraph Display */}
      <div className="reading-text">{renderWordHighlights()}</div>

      {/* Recorder */}
      <div className="recorder-container" style={{ marginTop: 24 }}>
        {isRecording && (
          <div className="waveform-container">
            {[...Array(10)].map((_, i) => <div key={i} className="waveform-bar" />)}
          </div>
        )}
        <button 
          className={`recorder-btn ${isRecording ? 'recording' : 'idle'}`}
          onClick={isRecording ? handleStopRecording : handleStartRecording} 
          disabled={analyzing || isSubmitting || submitted}
          style={{
            opacity: (analyzing || isSubmitting || submitted) ? 0.5 : 1,
            cursor: (analyzing || isSubmitting || submitted) ? 'not-allowed' : 'pointer',
          }}>
          {isRecording ? <Square size={28} /> : <Mic size={28} />}
        </button>
        <div className="recorder-timer">{formatDuration(duration)}</div>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: 8 }}>
          {isRecording ? 'Recording... Click to stop' : audioBlob ? 'Recording complete' : 'Click to start recording'}
        </p>

        {audioUrl && (
          <div style={{ marginTop: 16 }}>
            <div className="audio-player">
              <audio controls src={audioUrl} style={{ width: '100%', height: 40 }} />
            </div>
          </div>
        )}

        {transcript && (
          <div style={{ marginTop: 16, textAlign: 'left', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', maxHeight: 120, overflowY: 'auto' }}>
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 4 }}>Your speech (transcript):</p>
            <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>{transcript}</p>
          </div>
        )}

        <div className="recorder-controls" style={{ marginTop: 20 }}>
          {audioBlob && !analyzing && !submitted && (
            <>
              <button className="btn btn-secondary" onClick={handleReRecord} disabled={isSubmitting}>
                <RotateCcw size={16} /> Re-record
              </button>
              <button 
                className="btn btn-primary" 
                onClick={handleSubmit}
                disabled={!transcript || isSubmitting}
              >
                {isSubmitting ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Send size={16} />}
                {isSubmitting ? ' Submitting...' : ' Submit for Analysis'}
              </button>
            </>
          )}
        </div>

        {analyzing && (
          <div style={{ marginTop: 24, textAlign: 'center' }}>
            <Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: 'var(--accent-blue)' }} />
            <p style={{ color: 'var(--text-secondary)', marginTop: 8 }}>AI is analyzing your speech...</p>
          </div>
        )}
      </div>

      {/* Results */}
      {result && (
        <div className="result-section" style={{ marginTop: 32 }}>
          <div className="glass-card" style={{ padding: 32 }}>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              <div className="overall-score">{result.overallScore || 0}</div>
              <p style={{ color: 'var(--text-secondary)' }}>Reading Score</p>
              <span className={`performance-badge ${String(result.performanceLevel || 'Beginner').toLowerCase()}`}>
                {result.performanceLevel || 'Beginner'}
              </span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              {[
                { label: 'Pronunciation', score: result.pronunciation || 0, color: '#3b82f6' },
                { label: 'Fluency', score: result.fluency || 0, color: '#10b981' },
                { label: 'Grammar Clarity', score: result.grammarClarity || 0, color: '#8b5cf6' },
                { label: 'Speech Speed', score: result.speechSpeed || 0, color: '#06b6d4' },
                { label: 'Confidence', score: result.confidence || 0, color: '#f59e0b' },
                { label: 'Accent Clarity', score: result.accentClarity || 0, color: '#ec4899' },
                { label: 'Word Stress', score: result.wordStress || 0, color: '#f43f5e' },
                { label: 'Pauses', score: result.pausesHesitation || 0, color: '#64748b' },
              ].map((s, i) => (
                <div key={i} className="skill-bar">
                  <span className="skill-bar-label">{s.label}</span>
                  <div className="skill-bar-track">
                    <div className="skill-bar-fill" style={{ width: `${Math.min(s.score, 100)}%`, background: s.color }} />
                  </div>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem', color: s.color, minWidth: 30 }}>{Math.min(s.score, 100)}</span>
                </div>
              ))}
            </div>
            {Array.isArray(result.tips) && result.tips.length > 0 && (
              <div style={{ marginTop: 24, padding: 20, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)' }}>
                <h4 style={{ marginBottom: 12, color: 'var(--accent-amber)' }}>💡 Improvement Tips</h4>
                {result.tips.map((t, i) => (
                  <p key={i} style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: 8 }}>
                    • {t}
                  </p>
                ))}
              </div>
            )}
          </div>
        </div>
      )}
    </div>
  );
}